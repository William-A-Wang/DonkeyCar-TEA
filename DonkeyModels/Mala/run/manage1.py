...
class LineFollower:
    ...
    '''
    def __init__(self):
        self.scan_y = 150   # num pixels from the top to start horiz scan
        self.scan_height = 10 # num pixels high to grab from horiz scan
        self.color_thr_low = np.asarray((0, 117, 0)) # hsv dark yellow
        self.color_thr_hi = np.asarray((179, 255, 255)) # hsv light yellow
        ...
        self.pid_st = PID(Kp=0.01, Ki=0.00, Kd=0.00)

    
    def get_i_color(self, cam):
        '''
        ...
        iSlice = self.scan_y
        scan_line = cam[iSlice : iSlice + self.scan_height, :, :]
    ...
    def run(self, cam):
        ...
        max_yellow, confidense, mask = self.get_i_color(cam)
        conf_thresh = 0.001
        ...
        self.debug_display(cam, mask, max_yellow, confidense)

        return self.steering, self.throttle, self.recording


    def debug_display(self, cam, mask, max_yellow, confidense):
        '''
        ...
        display_str.append("Orange:{:d}".format(max_yellow))
...
if __name__ == '__main__':
    args = docopt(__doc__)
    cfg = dk.load_config(config_path='myconfig1.py')
    #cfg = dk.load_config(config_path='~/project/donkeycar/donkeycar/templates/cfg_path_follow.py')
    drive(cfg, args)
