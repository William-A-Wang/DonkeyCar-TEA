...
if __name__ == '__main__':
    ...
    ret = True
    # load model
    model = tf.keras.models.load_model('/home/donkeycar1/mycar/models/LSTM_model_v1_3imgs.h5', compile=False)
    ...
    while True:
        ...
        if len(image_group) == 3:
            print("Image Group Shape:", np.array(image_group).shape)
            print("Image Group Count:", len(image_group))

            # change sequence
            img_sequence = np.array(image_group)
            img_sequence = np.reshape(img_sequence, (1, 3, 144, 224, 1))
            print("Image Sequence Shape:", img_sequence.shape)
            # predict
            out = model.predict(img_sequence)
            print("Numbers:", len(out))
            print("Shape:", out.shape)
            out = out[-1,-1,:]
            print(out)
            throttle = 0.48  # (out[0][0][0])
            steering = np.argmax(out, axis=0) - 1
            print(steering)
            ...
            if throttle > 0:
                ...
                throttle_pulse = dk.utils.map_range(throttle, -1, 0, THROTTLE_REVERSE_PWM, THROTTLE_STOPPED_PWM)

            steering_pulse = dk.utils.map_range(steering, -1, 1, STEERING_LEFT_PWM, STEERING_RIGHT_PWM)
