...
if __name__ == '__main__':
     ...
     model_path = "/home/donkeycar1/mycar/models/gray_regression_newmean_e3000_b20_mse_Sun_Oct_29_20_27_41_2023.h5"
     ret = True   
     ...
     image_save_folder = time.ctime(time.time())
     os.mkdir(os.path.join("ZJ_data", image_save_folder))
     ...
     while True:
         ...
         img = np.dot(img[..., :3],
                [0.299, 0.587, 0.114])[height:224,:]  # Turn into grayscale (more realistic)
         out = model(img[None,:])
         
         throttle = new_throttle #(out[0][0][0].numpy())


         #steering = np.argmax(out,axis = 1) #- 1
         steering = out[0][0]
         
         cv2.imwrite(f"/home/donkeycar1/mycar/ZJ_data/{image_save_folder}/{str(count)}_{steering}.jpg", img_raw)
